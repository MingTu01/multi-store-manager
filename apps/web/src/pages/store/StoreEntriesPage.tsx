...
export default function StoreEntriesPage() {
  ...
  const load = () => {
    if (!storeId) return;
    api.get('/stores/' + storeId + '/entries?page=' + page + '&pageSize=' + pageSize + (isNonAdmin ? '&period=day' : '')).then((d) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`grid grid-cols-1 gap-3 ${isNonAdmin ? "sm:grid-cols-2" : "sm:grid-cols-3"}`}>
        ...
        {!isNonAdmin && (        <GlassCard className="p-4 text-center">
        ...
        {entries.length === 0 ? (
          ...
          <div key={e.id} className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {!isNonAdmin && (<button onClick={() => openEdit(e)} className="flex h-7 w-7 items-center justify-center rounded-lg hover:bg-slate-100"><Edit3 className="h-3.5 w-3.5 text-slate-400" /></button>)}
              {!isNonAdmin && (<button onClick={() => handleDelete(e.id)} className="flex h-7 w-7 items-center justify-center rounded-lg hover:bg-rose-50"><Trash2 className="h-3.5 w-3.5 text-rose-400" /></button>)}
            </div>
          </div>
        ))}
      </GlassCard>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white/60 px-4 py-2.5 text-xs text-slate-500 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          <span>每页</span>
          <select value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs outline-none">
            <option value={10}>10</option><option value={20}>20</option><option value={50}>50</option><option value={100}>100</option>
          </select>
          <span>条</span>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1} className="rounded px-2 py-1 hover:bg-slate-100 disabled:opacity-30">‹</button>
          <span>{page} / {Math.max(1, Math.ceil(total / pageSize))}</span>
          <button onClick={() => setPage(p => Math.min(Math.ceil(total / pageSize), p + 1))} disabled={page >= Math.ceil(total / pageSize)} className="rounded px-2 py-1 hover:bg-slate-100 disabled:opacity-30">›</button>
        </div>
      </div>
