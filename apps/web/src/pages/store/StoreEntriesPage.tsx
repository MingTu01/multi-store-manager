...
export default function StoreEntriesPage() {
  ...
  const load = () => {
    if (!storeId) return;
    api.get('/stores/' + storeId + '/entries?page=' + page + '&pageSize=' + pageSize).then((d) => {
        ...
        {entries.length === 0 ? (
          ...
          <div key={e.id} className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => openEdit(e)} className="flex h-7 w-7 items-center justify-center rounded-lg hover:bg-slate-100"><Edit3 className="h-3.5 w-3.5 text-slate-400" /></button>
              <button onClick={() => handleDelete(e.id)} className="flex h-7 w-7 items-center justify-center rounded-lg hover:bg-rose-50"><Trash2 className="h-3.5 w-3.5 text-rose-400" /></button>
            </div>
          </div>
        ))}
      </GlassCard>
