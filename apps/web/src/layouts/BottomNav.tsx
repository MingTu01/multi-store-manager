...
import { LayoutDashboard, Store, Bell, Settings, BookOpen, Package, Clock, BarChart3, Users, DollarSign, Divide, FileText, MoreHorizontal, X } from 'lucide-react';
...
export function BottomNav() {
  ...
  const adminTabs = [
    ...
    { to: '/admin-settings', icon: Users, label: '我的', key: 'adminSettings' },
  ];
  ...
  const navItem = (t: any, compact?: boolean) => (
    <NavLink
      key={t.to}
      to={t.to}
      end={'end' in t && t.end === true ? true : undefined}
      onClick={() => setShowMore(false)}
      className={({ isActive }) =>
        'flex flex-shrink-0 flex-col items-center gap-0.5 py-2 pt-2.5 text-[10px] transition-colors ' +
        (compact ? 'min-w-[64px] px-2' : 'min-w-[56px] px-1') +
            ...
            {filteredMore.map(t => (
              <NavLink
                ...
                className={({ isActive }) =>
                  ...
                  (isActive ? 'bg-indigo-50 font-semibold text-indigo-700' : 'text-slate-600 hover:bg-white/60')
                }
              >
        ...
        <div className="flex items-end justify-around">
          ...
          {filteredMore.length > 0 && (
            <button
              ...
              className="flex flex-shrink-0 flex-col items-center gap-0.5 px-1 py-2 pt-2.5 text-[10px] text-slate-400 transition-colors min-w-[56px]"
            >
