...
router.post('/', (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = db.prepare('INSERT INTO users (username, password_hash, name, phone, role, store_id, avatar, salary, status, job_title, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(username, hash, name || '', phone || '', role || 'staff', store_id || null, avatar || '', salary || 0, status || 'active', job_title || '', address || '');
...
router.put('/:id', (req: AuthRequest, res: Response) => {
  try {
    ...
    const params: any[] = [name, phone, role, store_id, avatar, salary, status, job_title, address, 'now', 'localtime'];
