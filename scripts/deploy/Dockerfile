...
# 注意：src-seed 不放在镜像层，而是放在 /app/data/src-seed（命名 volume，持久化）
# 原因：如果 src-seed 在镜像层，容器 down/up 后 src-seed 也会回退到镜像版本，
# 与 src 一起回退，无法起到恢复作用。放在 data volume 才能持久化升级后的版本。
COPY msl.js ./msl.js
